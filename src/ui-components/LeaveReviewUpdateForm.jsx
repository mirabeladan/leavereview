...
export default function LeaveReviewUpdateForm(props) {
  const {
    id: idProp,
    leaveReview: leaveReviewModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = leaveReviewRecord
      ? { ...initialValues, ...leaveReviewRecord }
      : initialValues;
  ...
  const [leaveReviewRecord, setLeaveReviewRecord] =
    React.useState(leaveReviewModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(LeaveReview, idProp)
        : leaveReviewModelProp;
      setLeaveReviewRecord(record);
    };
    queryData();
  }, [idProp, leaveReviewModelProp]);
  React.useEffect(resetStateValues, [leaveReviewRecord]);
  const validations = {
    Field0: [],
    Field1: [],
    Field2: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            LeaveReview.copyOf(leaveReviewRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "LeaveReviewUpdateForm")}
      {...rest}
    >
      <TextField
        label="Field0"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || leaveReviewModelProp)}
